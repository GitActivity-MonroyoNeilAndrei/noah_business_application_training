...
function cust_GetPara() {
    ...
    nwParameter_Add("cmbTowerPhase", $("#cmbTowerPhase").val());
}


//EVENTS
$(document).on("click", ".btn btn-cancel", function () {
...
$(document).on('click', '#btnReseration', function () {
    ...
    window.location.href = GetCurrentURL() + '../SBReservationEntry?' + params;
});
...
$(document).on('click', '#btnHoldingUnit', function () {
    ...
    var isLTO = $("#isLTO").text();
    //$("#iFrame1").attr("src", "../../../SB/DocumentEntry/SBHoldingOfUnit/SBHoldingOfUnit.aspx?nwdev=p8dev&nwu=" + recuser + "&uc=" + unitcode + "&isBulk="+isBulk+"");
    var params = "nwu=" + recuser + "&uc=" + unitcode + "&nwTypeCode=" + typecode + "&nwClassCode=" + classcode + "&isBulk=" + isBulk + "&isLTO=" + isLTO + "";
    window.location.href = GetCurrentURL() + '../SBHoldingOfUnit?' /*+ params*/;
});
//Holding per unit
$(document).on('click', '.btn-hold', function () {
    ...
    if (($("#amtp").text() != "0.00" || $("#amtu").text() != "0.00" || $("#amte").text() != "0.00") && $('#btnHoldingUnit').attr('hasAccess') != "") {
        ...
        var isLTO = "0";
        if ($("#chkLTO").prop("checked") == true)
            isLTO = "1";
        else
            isLTO = "0";
        var params = "nwu=" + recuser + "&uc=" + unitcode + "&nwTypeCode=" + typecode + "&nwClassCode=" + classcode + "&isBulk=" + isBulk + "&isLTO=" + isLTO + "&nkpop=y&nwdev=p8dev";
        //$("#iFrame1").attr("src", "../../../SB/DocumentEntry/SBHoldingOfUnit/SBHoldingOfUnit.aspx?nwdev=p8dev&" + params + "");
        $("#iFrame1").attr("src", GetCurrentURL() + "../SBHoldingOfUnit?" + params + "");
        //window.location.href = '/FLI_SellerPortalV10/SBHoldingOfUnit?' + params;
    }
});

//Letter of Intent Per Unit -- Added by JMG 01/20/2023 2:27PM
$(document).on('click', '.btn-loi', function () {
    var msg = "";
    if ($("#amtp").text() == "0.00" || $("#amtp").text() == "...") {
        NoSellingPrice("Letter of Intent");
    }
    if ($('#btnLOIEntry').attr('hasAccess') == "") {
        NoAccess("Letter of Intent");
    }
    if (selectedUnits.length <= 1 && $('#cbBulk').prop("checked")==true)
    {
        HasOneBulk("Letter of Intent");
        return false;
    }
    ...
    if (($("#amtp").text() != "0.00" || $("#amtu").text() != "0.00" || $("#amte").text() != "0.00") && $('#btnHoldingUnit').attr('hasAccess') != "") {
        ...
        $("#iframetitle").text("Letter of Intent");
        $(document).find('#popup').fadeIn();
        var isBulk = "0";
        var ucloi = "";
        if ($("#cbBulk").prop("checked") == true)
        ...
        var params = "nwu=" + recuser + "&uc=" + ucloi + "&nwTypeCode=" + typecode + "&nwClassCode=" + classcode + "&isBulk=" + isBulk + "&isLTO=" + isLTO + "&nkpop=y&nwdev=p8dev";
        //$("#iFrame1").attr("src", "../../../SB/DocumentEntry/SBHoldingOfUnit/SBHoldingOfUnit.aspx?nwdev=p8dev&" + params + "");
        $("#iFrame1").attr("src", GetCurrentURL() + "../SBLetterOfIntent?" + params + "");
        //window.location.href = '/FLI_SellerPortalV10/SBHoldingOfUnit?' + params;
    }
});

//payment term
$(document).on('click', '#btnPaymentTerm', function () {
        ...
        $("#iframetitle").text("Payment Term");
        $(document).find('#popup').fadeIn();
      
        var params = "nwu=" + recuser + "&uc=" + unitcode + "&nwTypeCode=" + typecode + "&nwClassCode=" + classcode + "&nwProject=" + pcode + "&nkpop=y&nwdev=p8dev";
        //$("#iFrame1").attr("src", "../../../SB/DocumentEntry/SBHoldingOfUnit/SBHoldingOfUnit.aspx?nwdev=p8dev&" + params + "");
        $("#iFrame1").attr("src", GetCurrentURL() + "../SBPaymentTermOptions?" + params + "");
        //window.location.href = '/FLI_SellerPortalV10/SBHoldingOfUnit?' + params;
});
...
$(document).on('click', '.list-item.btn', function (e) {
    ...
    if ($("#cbRetail").prop("checked") == true)
    {
        ...
        func_ActionDriven("actGenerateForeignOwn", false);
        $(".pdlg-li").show();
...
$(document).on('change', '#cmbPropertyType,#cmbLocation,#cmbTowerPhase', function () {
    //var loc = $(this).val().replace(/\s/g, "");
    //$([document.documentElement, document.body]).animate({
    //    scrollTop: $("#titleList" + loc + "").offset().top
    //}, 1000);
    $(".row-list-c").remove();
    ctag = $("#btnHome").attr("tag");
    cust_GetPara();
...
var uprice = "";
//click unit block
$(document).on('click', '.lvl-c-cont', function () {
...
$(document).on("click", ".btnlinFloor", function () {
    ...
    if ($('.selected').find('.stat').hasClass('isCoordinate') == false)
    {
        ...
        if ($("#chkLTO").prop("checked") == true)
            nwParameter_Add("isLTO", "yes");
        else
            nwParameter_Add("isLTO", "no");
...
$(document).on("click", "#btnViewBulkDtls", function () {
    ...
    else
    {
        ...
        if ($("#chkLTO").prop("checked") == true)
            ...
            isLTO = "0";
        $("#iFrame1").attr("src", GetCurrentURL() + "../MWBulkDetails?nkpop=y&nwdev=p8dev&nwu=" + recuser + "&nwUnits=" + bunits + "&isLTO=" + isLTO + "");
    }
});
//Select Another Project
...
$(document).on('change', '#chkLTO', function (e) {
    $('.btnlinFloor.selected').click();
});

function MainLoadInitial()
{
    $(".row-c.row-list-c").html("");
    var tag = getParameterByName("nwtag");
    if (tag == "1") {
        $("#buttonsLOI").hide();
        $("#buttonsHome").show();
        $("#btnViewBulkDtls").show();
...
function GenerateProjectVR(jsonproj) {
    ...
    for (var j = 1; j <= jsonproj.length ; j++) {
        var ns = jsonproj[j - 1]["Link"]
        if(ns == null)
             ns="";
        var valns = ns.replace(/\s/g, '');
...
function GeneratePicture(projd,jspic)
{
    ...
    var value = "";
    if (pic.toLowerCase().includes(".jpg") || pic.toLowerCase().includes(".jpeg") || pic.toLowerCase().includes(".png") || pic.toLowerCase().includes(".gif") || pic.toLowerCase().includes(".jfif"))
    {
...
function AddPhaseTower(code, desc) {
    $("#cmbTowerPhase").append("<option value='" + code + "'>" + desc + "</option>")
}
...
function GenerateNonSetupUnits(unitjson,maxqueue,user)
{
    ...
    for (var i = 1; i <= unitjson.length ; i++)
    {
        ...
        if ($("#cbBulk").prop("checked") == true && ((queueno < maxqueue && stats == "008") || stats == "003"))
        {
            isb = "";
            isc = "lvl-c-cont";
            chck = "<input type='checkbox' class='chk chkUnits' id='chk" + code + "' code='" + code + "' uprice='" + uprice + "'><label class='lblchk' for='chk" + code + "' style='float:right;padding-right: 0px;padding-left: 25px;'></label>";
        }
        else
        {
            isb = "lvl-c-cont";
        ...
        $(".lvl-c-box").append("<div class='lvl-c " + isb + "' style='color:white;text-shadow: -1px 2px 3px rgba(0, 0, 0, .18);background-color:" + unitjson[i - 1]["ColorSchm"] + "' status= '" + stats + "' uprice='" + uprice + "' code='" + code + "'><div class='lvl-c-lbl "+isc+"' status= '" + stats + "' uprice='" + uprice + "' code='" + code + "'>" + unitjson[i - 1]["Unit"] + "</div>" + chck + "</div>");
    }
...
//Only has 1 bulk unit
function HasOneBulk(title) {
    pmsg = "";
    if ($("#cbBulk").prop("checked") == true)
        $(".pdlg-ul").hide();
    else
        $(".pdlg-li").hide();
    $(".pdlg-title").text(title);
    pmsg += "Cannot be continued. At least 2 units are required. \n";
    $(".messagecont").text(pmsg);
    $(".pdlg-ul#pdlg-msg").show();
    $(document).find('#pdlgProject').fadeIn();
}
