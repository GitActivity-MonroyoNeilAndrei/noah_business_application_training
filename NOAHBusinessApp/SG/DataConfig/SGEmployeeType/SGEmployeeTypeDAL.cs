...
{
    public class SGEmployeeTypeDAL : NoahWebLib.DatabaseHandler
    {
        nwSFObjects SFObjects = new nwSFObjects();
        #region STANDARD

        public string MenuItemCode = "SGEmployeeType"; // This is default parameter  for version
...
                                ", MenuItemCode, MenuItemVersion)
        ...
        //#FOR EXPORT
        public string LISTINGFILENAME = "Employee Type", GETCOMPANY = "Select CompanyName from SG.BIRCASConfig";
        public int LISTINGSTARTROW = 5;
        //# END

        public string tableName = "[SG].[EmployeeType_Monroyo]";
        public string spName = "[SG].[nsp_EmployeeType_Monroyo]";

        public string CurrentSelectedItem;                                                 //-- selected item in binding navigator

        public SGEmployeeTypeDAL(string ConnectionString, string ConnectionString2, string selectedItem)
        {
        ...
        {
            return string.Format(@"SELECT code[Code],description[Description] FROM SG.EmployeeType_Monroyo");
        }
