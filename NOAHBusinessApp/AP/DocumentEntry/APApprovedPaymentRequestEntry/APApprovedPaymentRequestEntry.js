...
function func_Reload() {
    crLnk = GetCurrentURL() + "APApprovedPaymentRequestEntry_Gateway";
    crLnkGateKey = "APApprovedPaymentRequestEntry";
...
function ClearFields() {
    ...
    $("#txtDatePosted").val("");
    $("#txtApDmStatus").val("");
    $("#lugRsnDisapproval").val("");
    $("#txtDissRemarks").val("");
    $("#btnDocumentAttached").val("");
}
...
function nwGrid_AddtoListDone(nwGridID, crnwTRtemp, addtoListTableRec, index) {
    ...
    if (nwGridID == 'nwGridCon') {
        ...
        if (_col == (SPR_DOCNO - 1)) {
            ...
            var desc = addtoListTableRec.find('tr:eq(' + index + ') td:eq(2)').text();
            //if (col == SPR_USERCODE - 1) {
            var data = Grid.GetValue(SPR_ITEMCODE - 1, _row);
            var hasValue = false;

            if (index == 0 && data == "") {
                for (coli = 0; coli < collength; coli++) {
                    if ((SPR_ITEMCODE - 1) == coli
                        || (SPR_ITEMDESC - 1) == coli) {
                        continue;
                    }
                    var value = Grid.GetValue(coli, _row);
...
function Lookup_DoneFunction(idName, idNum) {
    ...
    if (idName == "lugPayee") {    
        ...
        if (isbuttonclick) {
            ...
            var subPayeeCode = getGridData(idNum, 2);
            var subPayeeDesc = getGridData(idNum, 3);
            var currencyCode = getGridData(idNum, 4);
            var currencyDesc = getGridData(idNum, 5);
            var checkPayeeName = getGridData(idNum, 6);


            $('#idvallugSubPayee').val(subPayeeCode);
            $('#descvallugSubPayee').val(subPayeeDesc);
            
            $('#idvallugCurrency').val(currencyCode);
            $('#descvallugCurrency').val(currencyDesc);

            $('#txtCheckPayeeName').val(checkPayeeName);

            // $('#idvallugSubPayee').val("neil");
            // $('#idvallugCurrency').val("neil2");
            // $('#txtCheckPayeeName').val("neil3");
            console.log(2)
...
function HasDataRemarks()
{
    ...
    setTimeout(function () {
        nwGridCon_Book.ActiveSheet.SetTemplate((SPR_REVIEWATTACHMENTS - 1), Spread_ALLROW, "remarks", "");
        nwGridCon_Book.ActiveSheet.SetText2((SPR_REVIEWATTACHMENTS - 1), Spread_ALLROW, "...");
        nwGridCon_Book.ActiveSheet.SetTextColor((SPR_REVIEWATTACHMENTS - 1), Spread_ALLROW, "#FFFFFF");
        nwGridCon_Book.ActiveSheet.SetBackground((SPR_REVIEWATTACHMENTS - 1), Spread_ALLROW, "#1130c7");

        for (var x = 1; x <= maxRow; x++) {
            var Remarks = nwGridCon_Book.ActiveSheet.GetText((SPR_TAGREMARKS - 1), x);

            if (Remarks == "1") {
                nwGridCon_Book.ActiveSheet.SetBackground((SPR_REVIEWATTACHMENTS - 1), x, "#006060");
            }
            else {
                nwGridCon_Book.ActiveSheet.SetBackground((SPR_REVIEWATTACHMENTS - 1), x, "#1130c7");
            }
        }
    }, 150);
}
...
function p8Spread_DblClick(canvasID, row, col) {
    ...
    if (canvasID == "nwGridCon") {
        ...
        if (col == (SPR_ITEMGROUPTYPECODE - 1)) {
            ...
            var item = '';
            for (var x = 0; x <= maxRow; x++) {
                item += "|" + nwGridCon_Book.ActiveSheet.GetText((SPR_ITEMGROUPTYPECODE - 1), x);
            }
            nwParameter_Add("txtItemCode", item);
