...
function func_Reload() {
    ...
    crLnk = "../SBMyProspectCustomers_HealthDeclaration/SBMyProspectCustomers_HealthDeclaration_Gateway";
    crLnkGateKey = "SBMyProspectCustomers_HealthDeclaration";
    ...
    $("#noah-webui-Toolbox").btn-tb-process().visible(true);
    DisableFields();
    
    if (getParameterByName("nwCus") != "") {
        $("#txtHCode").val(getParameterByName("nwCus"));
    }
    if (getParameterByName("nwPCus") != "") {
        $("#txtHPCode").val(getParameterByName("nwPCus"));
    }
...
function func_ToolboxSave(indef, enume) {
    ...
    parent_MessageBoxQuestionToolBox("Would you like to save the current record(s)?", "My Prospect Customers", "", indef, enume);
    isContinue = false;
...
function func_ToolboxDelete(indef, enume) {
    var isContinue = true;
    cust_GetPara();
    parent_MessageBoxQuestionToolBox("Would you like to delete the current record(s)?", "My Prospect Customers", "", indef, enume);
    isContinue = false;
    return isContinue;
}
...
function func_ToolboxProcess(indef, enume) {
    var isContinue = true;
    cust_GetPara();
    parent_MessageBoxQuestionToolBox("Do you want to process the current record?", "Health Declaration", "", indef, enume);
    isContinue = false;
    return isContinue;
}
...
function cust_GetPara() {
    nwParameter_Add("txtDateToday", $("#txtDateToday").val());
    nwParameter_Add("txtBodyTemp", $("#txtBodyTemp").val());
    nwParameter_Add("cbYes", $("#cbYes").prop("checked"));
    nwParameter_Add("cbNo", $("#cbNo").prop("checked"));

    nwParameter_Add("cbYes1", $("#cbYes1").prop("checked"));
    nwParameter_Add("cbNo1", $("#cbNo1").prop("checked"));

    nwParameter_Add("cbYes2", $("#cbYes2").prop("checked"));
    nwParameter_Add("cbNo2", $("#cbNo2").prop("checked"));

    nwParameter_Add("cmbSicknessPI", $("#cmbSicknessPI").val());
    nwParameter_Add("txtOthersSickness", $("#txtOthersSickness").val());
    nwParameter_Add("txtOthersLocation", $("#txtOthersLocation").val());
    nwParameter_Add("txtOthersDate", $("#txtOthersDate").val());
    nwParameter_Add("txtHCode", $("#txtHCode").val());
    nwParameter_Add("txtHPCode", $("#txtHPCode").val());
...
function DisableFields() {
    $("#txtDateToday").enable(false);
    $("#txtBodyTemp").enable(false);
    $("#cbYes").enable(false);
    $("#cbNo").enable(false);
    //$("#cbYes").prop("checked", false);
    //$("#cbNo").prop("checked", false);
    $("#cmbSicknessPI").enable(false);
    $("#txtOthersSickness").enable(false);
    $("#cbYes1").enable(false);
    $("#cbNo1").enable(false);
    $("#txtOthersLocation").enable(false);
    $("#cbYes2").enable(false);
    $("#cbNo2").enable(false);
    $("#txtOthersDate").enable(false);

    $("#txtDateToday").val('');
    $("#txtBodyTemp").val('');
    $("#cmbSicknessPI").val('');
    $("#txtOthersSickness").val('');
    $("#txtOthersLocation").val('');
    $("#txtOthersDate").val('');
    //$("#dtpTranDate").enable(false)
    ...
    //$("#noah-webui-Toolbox").bindingExport().visible(true);
    //$("#noah-webui-Toolbox").bindingExport().enable(false);

    //$("#txtEmailAdd").enable(true);
    //$("#txtMobileNum").enable(true);

    //$("#noah-webui-Toolbox").bindingRefresh().enable(true);
    //$("#noah-webui-Toolbox").btn-tb-process().enable(false);
...
function EnableFieldsDone() {//Binding Done
    ...
    $("#cmbSicknessPI").enable(false);
    $("#txtOthersSickness").enable(false);
    $("#cbYes1").prop("checked", false);
    $("#cbNo1").prop("checked", false);
    $("#txtOthersLocation").enable(false);
    $("#cbYes2").prop("checked", false);
    $("#cbNo2").prop("checked", false);
    $("#txtOthersDate").enable(false);
...
function func_WindowCloseTrigger(verID) {
    if (verID == "nwUploadCon") {
        if ($btn == "attachIDPicture") {
            ...
            path = "\\" + 'SBMyProspectCustomers_SendLink' + "\\" + filepath;
...
$(document).on("change", ".txtBday", function (e) {
    ...
    if (bday > csd) {
        MessageBox("Cannot be continued. Future date is not allowed.", "My Prospect Customers");
        $("#" + id).val('');
        $("#" + age).val('');
    }
...
$(document).on('change', '#txtEffectiveDateToAF', function () {
    ...
    if (Date.parse(effectivetyDate) < Date.parse(currServerDate)) {
        MessageBox("Cannot Proceed. Effective Date To should not be earlier than Effective Date From.", "My Prospect Customers");
...
$(document).on('change', '#cmbSicknessPI', function () {
    CheckIfOthers("cmbSicknessPI");
});
...
//$(".nwlookupgridList ").find(`tr:eq(3)`).find("td:eq(0)").find("[type='checkbox']").prop('checked');
...
function func_SickToday(sick) {
    switch (sick) {
        case "Y":
           
            $('#cmbSicknessPI').enable(true);
            //$('#txtOthersSickness').enable(true);
            break;
        case "N":
...
function func_Traveled(trav) {
    switch (trav) {
        case "Y":

            //$('#cmbSickness').enable(true);
            $('#txtOthersLocation').enable(true);
            break;
        case "N":

            //$('#cmbSickness').enable(false);
            $('#txtOthersLocation').enable(false);
            $('#txtOthersLocation').val('');
            break;
        default:
...
function func_CloseCont(cont) {
    switch (cont) {
        case "Y":

            //$('#cmbSickness').enable(true);
            $('#txtOthersDate').enable(true);
            break;
        case "N":
...
$(document).on('focusout', '.txtTIN', function () {
    ...
    if ($("#cbIndividual").prop("checked") == true) {
        ...
        for (var i = 0; i < tinvalues.length; i++) {
            if ($("#" + tinvalues[i]).val() == $("#" + id).val() && id != tinvalues[i]) {
                MessageBox("Cannot be continued. TIN is already existing.", "My Prospect Customers");
                $("#" + id).val('');
            }
    ...
    //if ($("#cbIndividual").prop("checked") == true)
    //{
    //    if ($("#" + id).val() == $("#txtTIN").val() && id != "txtTIN") {
    //        MessageBox("Cannot be continued. TIN No. already exists.", "My Prospect Customers");
    //    }
    //    if ($("#" + id).val() == $("#txtTIN").val() && id != "txtTINSP") {
    //        MessageBox("Cannot be continued. TIN No. already exists.", "My Prospect Customers");
    //    }
    //    if ($("#" + id).val() == $("#txtTIN").val() && id != "txtTINCO") {
    //        MessageBox("Cannot be continued. TIN No. already exists.", "My Prospect Customers");
    //    }
    //    if ($("#" + id).val() == $("#txtTIN").val() && id != "txtTINCOS") {
    //        MessageBox("Cannot be continued. TIN No. already exists.", "My Prospect Customers");
    //    }
    //}
...
function checkCheckBoxLookupList() {

    var tableName = $(".nwlookupgridList ");
    var indexValie = $("#txtDesiredPropertyPI").val();
    var seperatedVal = indexValie.split(/[;]+/)

    for (var i = 0 ; i < seperatedVal.length; i++) {

        console.log(seperatedVal[0])
        console.log([i])
        if (seperatedVal[i] == "OTHERS") {
            $("#txtOthersDesiredPI").enable(true);
        }

        //$('.nwlookupgridList  > tbody  > tr').each(function (index, tr) {

        //    tableName.find('tr:eq(' + seperatedVal[i] + ')').find("td:eq(0)").find("[type='checkbox']").prop('checked', true)
...
$(document).on('change', '#txtDateToday', function () {
    var currServerDate = $('#txtDateToday').val();
    $('#txtDateToday').val(currServerDate);
});
...
function PopupWindow(ID) {
    ...
    switch (ID) {
        case 1:
            ...
            fullength = "../Popups/SBMyProspectCustomers_SendLink_SendLink?"//nwType=1&isView=" + encodeURI(isView) + "&nwSellerCode=" + encodeURI(SellerCode) + "&nwSellerName=" + encodeURI(Sellername) + "&nwTranType=MOCR&nwtitle=" + encodeURI(title);
            break;
    }
