...
{
    public class APPaymentTermDAL : NoahWebLib.DatabaseHandler
    {
        nwSFObjects SFObjects = new nwSFObjects();
        #region STANDARD

        public string MenuItemCode = "APPaymentTerm"; // This is default parameter  for version
...
                                ", MenuItemCode, MenuItemVersion)
        ...
        //#FOR EXPORT
        public string LISTINGFILENAME = "Payment Term", GETCOMPANY = "Select CompanyName from SG.BIRCASConfig";
        public int LISTINGSTARTROW = 5;
        //# END

        public string tableName = "[AP].[PaymentTerm_Monroyo]";
        public string spName = "[AP].[nsp_PaymentTerm_Monroyo]";

        public string CurrentSelectedItem;                                                 //-- selected item in binding navigator

        public APPaymentTermDAL(string ConnectionString, string ConnectionString2, string selectedItem)
        {
        ...
        public string SaveData(DataTable dt, bool IsNewRow)
        {
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@NumberOfDays", dr["NumberOfDays"]);
        ...
        {
            return string.Format(@"SELECT code[Code],description[Description], numberofdays[NumberOfDays] FROM AP.PaymentTerm_Monroyo");
        }
