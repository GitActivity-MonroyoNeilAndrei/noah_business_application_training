...
/*
    Modified by: Angelo Carlo Gonzales
    For Noah Web / Promptus8 Web applications
    Pritting of PDF.
    # Date Used : November 2016
    # Date Modified : March 16, 2017 / 02:05 PM  - before: 11-29-2016
*/
...
function webViewerPrint() {
    ...
    if (url.indexOf("#printbutton") >= 1) isValidPrint = true;
    else isValidPrint = window.parent.nwPDFPrint("");

    if(isValidPrint==true)
        window.print();
...
var isValidDownload = true;
function webViewerDownload() {
     //nwaagedit
     
      var url =window.location+"";
    if (url.indexOf("#downloadbutton") >= 1) isValidPrint = true;
    else isValidPrint = window.parent.nwPDFDownload("");
...
window.addEventListener('beforeprint', function beforePrint(evt) {
    //try {
    //    var isValid = window.parent.Darat();
    //} catch (err) {
...
$(function () {
    ...
    function nwOnPrintLoad() {
        ...
        var isPrint=false;
        try {
            xindex = url.indexOf("#page=");
        } catch (err) {
        }

        try {
            xindexP = url.indexOf("#printbutton");
            if (xindexP <= 0) xindexP = url.length;
            else isPrint = true;
        } catch (err) { xindexP = url.length; }


      //  alert(url);
       
        //$("#pageNumberLabel").text(url);
        //$("#pageNumberLabel").attr("title", url);
        if (xindex >= 0) {
            url = url.substring(xindex + 6, xindexP);
        }
        ...
        if (isPrint) {
            ...
            setTimeout(function () {
              //  PDFViewerApplication.pdfViewer.currentPageNumber = parseInt(url);
            }, 300);
