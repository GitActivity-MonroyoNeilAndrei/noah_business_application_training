...
{
    public class RegistrationFormDAL : NoahWebLib.DatabaseHandler
    {
        nwSFObjects SFObjects = new nwSFObjects(); /// should be added
        #region STANDARD

        public string MenuItemCode = "RegistrationForm"; // This is default parameter  for version
...
                                ", MenuItemCode, MenuItemVersion)
        ...
        public RegistrationFormDAL(string ConnectionString, string ConnectionString2, string selectedItem)
        {
        ...
        {
            string strSQL = $@"EXEC [PRT].[nsp_RegistrationForm] @trantype = '{trantype}', @filter='{filter}' , 
        ...
        {
            string strSQL = $@"EXEC [PRT].[nsp_RegistrationForm] @trantype = '{trantype}', @docno = '{docno}', @QueryType = 21";
            return SFObjects.LoadDataTable(strSQL, _ConnectionString);
        }
        ...
        public string getLookup(string trantype, string project, string unitCode, string customerCode,
                        string customerClass, string sourceOfSale, string recUser, string filter)
        {
            return string.Format($@"EXEC [PRT].[nsp_RegistrationForm] @trantype = '{trantype}', @filter='{filter}' , 
                                @project = '{project}' , @UnitCode = '{unitCode}' , @customerCode='{customerCode}' , 
                                @customerClassification = '{customerClass}' , @sourceOfSale = '{sourceOfSale}', @recuser='{recUser}',
                                @QueryType = 20
                            ");
