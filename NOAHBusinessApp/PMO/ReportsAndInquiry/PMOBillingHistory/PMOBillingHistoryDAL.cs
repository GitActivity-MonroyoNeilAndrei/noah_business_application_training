...
{
    public class PMOBillingHistoryDAL : NoahWebLib.DatabaseHandler
    {
        nwSFObjects SFObjects = new nwSFObjects();
        #region STANDARD

        public string MenuItemCode = "PMOBillingHistory"; // This is default parameter  for version
...
                                ", MenuItemCode, MenuItemVersion)
        ...
        public PMOBillingHistoryDAL(string ConnectionString, string ConnectionString2, string selectedItem)
        {
        ...
        public string ProcessData(DataTable dt, string user)
        {
            try
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    cmd.CommandText = "[PRT].[nsp_PMOBillingHistory]";
        ...
        {
            SqlCommand cmd = new SqlCommand();
            cmd.CommandText = "[PMO].[nsp_BillingHistorySOA]";
            cmd.Parameters.Clear();
            ...
            string query = string.Format($@"EXEC [PMO].[nsp_BillingHistorySOA] @QueryType = 0, 
        ...
        //{
        //    return string.Format($@"Exec [PRT].[nsp_PMOBillingHistory] @QueryType = 20, @CustClassList='{custClassList}'");
        //}

        //public string InquireCustClassList(string customerList)
        //{
        //    return string.Format($@"Exec [PRT].[nsp_PMOBillingHistory] @QueryType = 21, @CustomerList='{customerList}'");
        //}
        ...
        {
            return string.Format($@"Exec [PRT].[nsp_BillingHistory] @QueryType = 20");
        }

        public DataTable LoadSchema(string customerList, string custClassList)
        {
            SqlCommand cmd = new SqlCommand();
            cmd.CommandText = "[PRT].[nsp_BillingHistory]";
            cmd.Parameters.Clear();
