...
baseTitle = "Vendor Item Taxes Assignment";
...
function func_ToolboxSave(indef, enume) {
    ...
    parent_MessageBoxQuestionToolBox("Do you want to save the current record?", 'Vendor Item Taxes Assignment', "", indef, enume);
    isContinue = false;
...
function func_ToolboxProcess(indef, enume) {
    var isContinue = true;
    cust_GetPara();
    parent_MessageBoxQuestionToolBox("Do you want to process the current record?", 'Vendor Item Taxes Assignment', "", indef, enume);
    isContinue = false;
...
function func_ToolboxDelete(indef, enume) {
    var isContinue = true;
    cust_GetPara();
    parent_MessageBoxQuestionToolBox("Do you want to delete the current record?", 'Vendor Item Taxes Assignment', "", indef, enume);
    isContinue = false;
    return isContinue;
}
    ...
    if (nwGridID == 'nwGridCon') {
        if (_col == (SPR_ITEMGROUPTYPECODE - 1)) {
            ...
            if (hasValue) {
                ...
                Grid.SetText((SPR_BASEUOMCODE - 1), _row, addtoListTableRec.find('tr:eq(' + index + ') td:eq(6)').text());
                ...
                crnwTRtemp[SPR_BASEUOMCODE - 1] = addtoListTableRec.find('tr:eq(' + index + ') td:eq(6)').text();
...
function nwGrdi_DblClick(nwGridObj, crTR, crTD) {
    ...
    if (nwGridObj.attr("id") == "nwGrid") {
        ...
        if (crnwTD.index() == SPR_ITEMGROUPTYPECODE) {

            var maxRow = $('#nwGridCon .tblGridBody tr').length;
            var item = '';
            for (var x = 0; x <= maxRow; x++) {
                item += "|" + $('#nwGridCon .tblGridBody  tr:eq(' + x + ')').find('td:eq(' + SPR_ITEMCODE + ')').text();
            }
            nwParameter_Add("txtItemCode", item);
            nwParameter_Add("txtItemGroupTypeCode", nwLib.nwTempTable_RowData_Get("nwGrid", crnwTR.index(), SPR_ITEMGROUPTYPECODE));
            lookUpCustomize("lugItemGroupTypeCode", 2);
        ...
        if (crnwTD.index() == SPR_VATTAXDESC) {

            nwParameter_Add("txtVattaxCode", nwLib.nwTempTable_RowData_Get("nwGrid", crnwTR.index(), SPR_VATTAXDESC));
            lookUpCustomize("lugVattaxCode", 1);

        }

        if (crnwTD.index() == SPR_EWTDESC) {

            nwParameter_Add("txtEWTCode", nwLib.nwTempTable_RowData_Get("nwGrid", crnwTR.index(), SPR_EWTDESC));
...
function InsertRow() {
    ...
    for (var y = 0 ; y <= maxRow; y++) {
        var OTH = nwGridCon_Book.ActiveSheet.GetText((SPR_ITEMGROUPTYPECODE - 1), y);
        if (OTH != "") {
            _count++
        }
    }
...
function EnableButtonReqCom()
{
    var maxRow = nwGridCon_Book.ActiveSheet.GetMaxRow();
    var tranNo = $("#txtTransactionNo").val();

    for (var x = 0; x <= maxRow; x++)
    {

        var TagItemG = nwGridCon_Book.ActiveSheet.GetText((SPR_ITEMGROUPTYPECODE - 1), x);

        if (TagItemG != "" && tranNo != "")
        {
            nwGridCon_Book.ActiveSheet.SetEnable((SPR_REQCOM - 1), x, true);
        }

        else
        {
            nwGridCon_Book.ActiveSheet.SetEnable((SPR_REQCOM - 1), x, false);
        }
    }
}


$(document).on("keydown", ".txtSpreadRemarks", function (e) {
    var code = e.which;
    if (code == "13") {
        if ($("#chkSpreadRemarks").is(':checked')) {
            var gridIDCon = crnwTable.attr('id');

            if ($('#txtSpreadRemarks').val() != '') {
                //$(`#${gridIDCon} tr:eq(${crnwTR.index()})`).find(`td:eq(${SPR_REMARKS}) button`).addClass("hasData");
                nwGridCon_Book.ActiveSheet.SetBackground((SPR_REMARKS - 1), _row, "#006060");
            }
            else {
                //$(`#${gridIDCon} tr:eq(${crnwTR.index()})`).find(`td:eq(${SPR_REMARKS}) button`).removeClass("hasData");
                nwGridCon_Book.ActiveSheet.SetBackground((SPR_REMARKS - 1), _row, "#1130c7");
            }
        }
    }
});
...
function HasDataRemarks()
{
    ...
    setTimeout(function () {
        ...
        for (var x = 1; x <= maxRow; x++) {
            ...
            if (Remarks == "1") {
                //$(`#nwGridCon tr:eq(${x})`).find(`td:eq(${SPR_REMARKS}) button`).addClass("hasData");
                nwGridCon_Book.ActiveSheet.SetBackground((SPR_REMARKS - 1), x, "#006060");
            }
            else {
                //$(`#nwGridCon tr:eq(${x})`).find(`td:eq(${SPR_REMARKS}) button`).removeClass("hasData");
...
$(document).on("click", '#btnSpreadRemarks', function (e) {
    var gridIDCon = crnwTable.attr('id');
...
$(document).on("keydown", "#spreadRemarksCon", function (e) {
    var code = e.which;
    if (code == "13") {
