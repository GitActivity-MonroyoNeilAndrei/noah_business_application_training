...
# Date Modified : November 03 2023 / 4:31 PM  - before: 08-18-2023
...
P8.Spread.prototype.SetSpreadConfig = function (jsonData) {
    ...
    for(var i = 0 ; i < json.length; i ++){
        ...
        this.Sheet[i].Data = json[i].Data;
        this.Sheet[i].ColumnConfig = json[i].ColumnConfig;
        this.Sheet[i].RowConfig = json[i].RowConfig;
    }
...
P8.Spread.prototype.SetThemes = function (theme) {
     if(P8Themes.FANCY == theme){
        ...
        //$("#" + canvasID+" .P8Spread_SheetHScroll").hide();
        //$("#" + canvasID+" .P8Spread_SheetVScroll").hide();
...
function _sfRenderFunction(_this) {
    ...
    if (_this.ScrollActive) {
        ...
        try { clearTimeout(P8RendeVar); } catch (err) {  console.log("error:" + err); }
...
function canvasCreateListHTML(canvasID, obj) {
    ...
    if ($("#" + canvasID).hasClass("nkListView")) {
        ...
        x_html += "<div class='nkListView' canvasID='"+containerID+"'>";
        x_html += "</div>";
        $('#' + containerID).html(x_html);
    ...
    for(var irow=startrow; irow < maxrow; irow++){
        ...
        for(var icol=0; icol < obj.ColumnConfig.length; icol++){
            ...
            if(obj.GetObjectType(icol,irow) == "buttonflat"){
                ...
                x_html += "</div>";
                x_html += "<div class='nkListViewValue "+(enabled ? "enable" : "" )+"' style='background-color"+background+";color:"+textcolor+";' >"; //<textarea "+(enabled ? "" : "disabled='disabled'")+" >
                x_html += obj.GetText(icol,irow);
                x_html += "</div>"; //</textarea>
            }
...
$(document).on("click",".nkListViewCol",function(){
    ...
    var row = $(this).parents(".nkListViewRow").attr("row");
    var col = $(this).index();
...
function _sfPaste(obj) {
    ...
    //setTimeout(function () {
    //    $('#' + obj.canvasID + '_vw_inpText').removeClass("paste");
    //}, 1300);
}
        ...
        if(P8DataList[canvasID][0].sheet.ActiveSheet.Theme  == P8Themes.FANCY)
        {
            ...
            if(isenabled){
                if (objecttype == "celltext") {
                    if($(_this).find("#p8SpreadListInput").val() == undefined){
                        $(_this).html("<input id='p8SpreadListInput' canvasID='"+canvasID+"' col='"+col+"' row='"+row+"' />");
...
$(document).on("focusout","#p8SpreadListInput",function(){
    var value = $(this).val();
