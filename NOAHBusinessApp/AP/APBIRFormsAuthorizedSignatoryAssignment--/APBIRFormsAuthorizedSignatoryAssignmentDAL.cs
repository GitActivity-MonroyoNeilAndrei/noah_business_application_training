...
{
    public class APBIRFormsAuthorizedSignatoryAssignmentDAL : NoahWebLib.DatabaseHandler
    {
        nwSFObjects SFObjects = new nwSFObjects(); /// should be added
        #region STANDARD

        public string MenuItemCode = "APBIRFormsAuthorizedSignatoryAssignment"; // This is default parameter  for version
...
                                ", MenuItemCode, MenuItemVersion)
        ...
        //#FOR EXPORT
        public string LISTINGFILENAME = "BIR Forms Authorized Signatory Assignment",
        ...
        private string storedProcedureName = "[AP].[nsp_BIRFormsAuSignAssignHDR_Monroyo]";
        private string storedProcedureNameLIN = "[AP].[nsp_BIRFormsAuSignAssignLIN_Monroyo]";

        private string tableName = "[AP].[BIRFormsAuSignAssignHDR_Monroyo]";
        private string tableNameLin = "[AP].[BIRFormsAuSignAssignLIN_Monroyo]";

        public APBIRFormsAuthorizedSignatoryAssignmentDAL(string ConnectionString, string ConnectionString2, string selectedItem)
        {
        ...
        public string BIRAuthoSign()
        {
            return string.Format(@"EXEC " + storedProcedureName + "@QueryType = 22");
        }
        ...
        public string SaveData(DataTable dtHDR, DataTable dtLIN, bool IsNewRow, string Trantype)
        {
            try
            {
                ...
                cmd.Parameters.AddWithValue("@Location", dtHDR.Rows[0]["Location"].ToString());
                cmd.Parameters.AddWithValue("@EffectiveDate", dtHDR.Rows[0]["EffectiveDate"].ToString());
                cmd.Parameters.AddWithValue("@BIRAuthoSign", dtHDR.Rows[0]["BIRAuthoSign"].ToString());
