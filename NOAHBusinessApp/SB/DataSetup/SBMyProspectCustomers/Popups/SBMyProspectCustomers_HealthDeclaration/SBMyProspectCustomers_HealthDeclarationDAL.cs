...
{
    public class SBMyProspectCustomers_HealthDeclarationDAL : DatabaseHandler
    {
        nwSFObjects SFObjects = new nwSFObjects(); /// should be added
        #region STANDARD

        public string MenuItemCode = "SBMyProspectCustomers_HealthDeclaration"; // This is default parameter  for version
        ...
        public string spName = "[PRT].[nsp_SBMyProspectCustomers]";
        ...
        //public string inquireQuery = "EXEC [PRT].[nsp_SBMyProspectCustomers_HealthDeclaration] @QueryType = 6, @recuser='{1}'", RecUser;
        public string inquireQuery(string RecUser)
        {
            return string.Format($@"EXEC {spName} @QueryType = 6, @recuser='{RecUser}'", _ConnectionString);
        }

        public SBMyProspectCustomers_HealthDeclarationDAL(string ConnectionString, string ConnectionString2, string selectedItem)
        {
        ...
        public string LISTINGQUERY(string recuser)
        {
            ...
            string sql = $"EXEC {spName} @QueryType = 29, @recuser='{recuser}'";
            return sql;
        ...
        {
            return SFObjects.LoadDataTable(string.Format("EXEC " + spName + " @CustomerCode='" + code + "',@QueryType=53"), _ConnectionString);
        }
        ...
        {
            return SFObjects.LoadDataSet(string.Format("EXEC " + spName + " @QueryType=11"), _ConnectionString);
        }
        ...
        {
            return SFObjects.LoadDataSet(string.Format("EXEC " + spName + " @QueryType=8"), _ConnectionString);
        }
        public DataTable GetComboValueSpecial(int qt, string code, string ft, string cust)
        {
            return SFObjects.LoadDataTable(string.Format("EXEC " + spName + " @Code='" + code + "',@FilterType='" + ft + "',@QueryType=" + qt + ",@CustomerCode='" + cust + "'"), _ConnectionString);
        }
        public DataTable GetCoownerDetails(string code)
        {
            return SFObjects.LoadDataTable(string.Format("EXEC " + spName + " @CustomerCode='" + code + "',@QueryType=14"), _ConnectionString);
        }
                ...
                switch (i)
                {
                    case 0://main
                        foreach (DataRow dr in dt.Rows)
                        {
                            ...
                            cmd.Parameters.AddWithValue("@AllowNotifIndivYes", dr["AllowNotifIndivYes"]);
                    ...
                    case 5://attorney lin
                        foreach (DataRow dr in dt.Rows)
                        {
                            ...
                            cmd.Parameters.AddWithValue("@chkRepresentative", dr["chkRepresentative"]);
                            cmd.Parameters.AddWithValue("@isUpdate", isNew ? 0 : 1);
                            cmd.Parameters.AddWithValue("@QueryType", 44);
                        }
                        break;
                    case 6://attorney contacts
                        foreach (DataRow dr in dt.Rows)
                        {
                            cmd.Parameters.AddWithValue("@CustomerCode", dr["ID"]);
                            cmd.Parameters.AddWithValue("@LandlineNo", dr["LandlineNo"]);
                    ...
                    case 9://health dec
                        ...
                        {
                            cmd.Parameters.AddWithValue("@CustomerCode", dr["CustomerCode"]);
                            cmd.Parameters.AddWithValue("@DateToday", dr["DateToday"]);
                            cmd.Parameters.AddWithValue("@Bodytemp", dr["Bodytemp"]);
                            cmd.Parameters.AddWithValue("@isSickToday", dr["isSickToday"]);
                            cmd.Parameters.AddWithValue("@Symptoms", dr["Symptoms"]);
                            cmd.Parameters.AddWithValue("@SymptomsOthers", dr["SymptomsOthers"]);
                            cmd.Parameters.AddWithValue("@SourceOfAwarenessOthers", dr["SymptomsOthers"]);
                            cmd.Parameters.AddWithValue("@isTraveled", dr["isTraveled"]);
                            cmd.Parameters.AddWithValue("@LocOthers", dr["LocOthers"]);
                            cmd.Parameters.AddWithValue("@isCloseContact", dr["isCloseContact"]);
                            cmd.Parameters.AddWithValue("@DateOthers", dr["DateOthers"]);
                            cmd.Parameters.AddWithValue("@isUpdate", isNew ? 0 : 1);
        ...
        public string ProcessData(string code,string DateToday, string BodyTemp, string Sickness, string OthersSickness, string OthersLocation, string OthersDate, int SickToday, int Traveled,int CloseCon, string PCode)//, string password, string customername)
        {
            ...
            cmd.Parameters.AddWithValue("@BodyTemp", BodyTemp);
            cmd.Parameters.AddWithValue("@isSickToday", SickToday);
            cmd.Parameters.AddWithValue("@Symptoms", Sickness);
            cmd.Parameters.AddWithValue("@SymptomsOthers", OthersSickness);
            cmd.Parameters.AddWithValue("@isTraveled", Traveled);
            cmd.Parameters.AddWithValue("@LocOthers", OthersLocation);
            cmd.Parameters.AddWithValue("@isCloseContact", CloseCon);
            cmd.Parameters.AddWithValue("@DateOthers", OthersDate);
            cmd.Parameters.AddWithValue("@ProspectCustomerCode", PCode);
        ...
        public string getSymp = string.Format("SELECT Code, Description FROM RE.SBMyProspectCustomers_Symptoms");
        ...
        public string getCivilStatus(string code)
        {
            ...
            return string.Format($@"EXEC {spName} @QueryType = 11, @salutation='{code}'", _ConnectionString);

            //return string.Format($@"EXEC [PRT].[nsp_SBMyProspectCustomers_HealthDeclaration] @QueryType = 23, @custtagging='{vip}'", _ConnectionString);
        }
        public string getCustClass()
        {

            return string.Format($@"EXEC {spName} @QueryType = 30", _ConnectionString);
        ...
        {
            return Parser.ParseInt(SFObjects.returnText(string.Format($@"exec {spName} @Querytype = 21, @Company = '{Company}'"), _ConnectionString)).Equals(1);
        }

        public DataTable CheckCustomer(string lastname, string firstname, string bday) //LAM
        {
            return SFObjects.LoadDataTable(string.Format($@"exec {spName} @Querytype = 22, @LastName = '{lastname}', @FirstName = '{firstname}', @Birthday = '{bday}'"), _ConnectionString);
        }
        ...
        public string getCustTagging()
        {
            ...
            string sql = string.Format(@"SELECT VIP,MI FROM  PRT.SBMyProspectCustomers_HealthDeclaration");
            return sql;
        }
        ...
        {
            return Parser.ParseInt(SFObjects.returnText(string.Format($@"EXEC {spName} @QueryType = 7 , @recuser = '{Code}'"), _ConnectionString));
        }
        ...
        {
            return SFObjects.returnText(string.Format($@"EXEC {spName} @QueryType = 19 , @RegisteredName = '{regname}', @Birthday = '{bday}'"), _ConnectionString);
        }
        ...
        {
            string sql = $"Exec {spName} @Querytype =25 , @CustomerCode='{CustomerCode}'";
            return SFObjects.returnText(sql, _ConnectionString);
        }
        public string getCustomerSignaturePath(string CustomerCode)
        {
            string sql = $"Exec {spName} @Querytype =27 , @CustomerCode='{CustomerCode}'";
            return SFObjects.returnText(sql, _ConnectionString);
        }

        public string validateTIN(string CustomerCode, string TIN)
        {
            string sql = $"Exec {spName} @Querytype =31 , @CustomerCode='{CustomerCode}', @TIN = '{TIN}'";
            return SFObjects.returnText(sql, _ConnectionString);
        }
        //public string validateTINSpouse(string TIN)
        //{
        //    string sql = $"Exec [PRT].[nsp_SBMyProspectCustomers_HealthDeclaration] @Querytype =31 , @CustomerCode='{CustomerCode}', @TIN = '{TIN}'";
        //    return SFObjects.returnText(sql, _ConnectionString);
        //}
        ...
        {
            string sql = $"Exec {spName} @Querytype ={qtype} , @CustomerCode='{CustomerCode}', @VatREGTin = '{TIN}'";
            return SFObjects.returnText(sql, _ConnectionString);
        }
        ...
        {
            string a = string.Format(@"Exec " + spName + " @Querytype =28 , @Salutation='{0}'", code);
