...
(function () {
    ...
    function setupShepherd() {
        ...
        var glength = 0;
        try{
            if(jsonGuide == undefined)jsonGuide =  []; 
            glength=jsonGuide.length;
        }catch(err){
            try{
                jsonGuide = [];
            }catch(err){
        ...
        if (glength  >= 1) {
            ...
            for (var i = 1; i < jsonGuide.length; i++) {
                x_index = i;
                obj = jsonGuide[x_index];
