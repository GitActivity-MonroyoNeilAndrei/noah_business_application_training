...
$(document).ready(function () {
    ...
    crLnk = "../Modules_UI/mainMenu/Login/Login_Gateway.aspx";
    ...
    func_ActionDriven("actInitial", false);
...
function LoadingStart() {
    ...
    nwParameter_Add("cbxAD", false); //to be follow
...
window.onload = function () {
    ...
    $(document).on('click', '#btnViewPolicy', function () {
        try {
            window.location.href = 'https://www.noahapplication.com/privacy-statement.html';
        } catch (err) {
...
$(document).ready(function () {
    ...
    setTimeout(function () {
        // Logo size (Login page)
        var $logo = $(document).find("#logocompLogo");
        if ($logo.outerHeight() > 54) {
            $logo.css({
                "height": "54px",
                "width": "initial",
            });
...
function Login() {
    try {
        window.location.href = '../MW/DocumentEntry/Main/default.aspx';
    } catch (err) {
